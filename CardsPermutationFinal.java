 ...
 */    public static long solve(int n, int[] perm) {
        ...
        undefinedAmnt[n - 1] = 0;
        for (int i = n - 2; i >= 0; i--) {
            undefinedAmnt[i] = undefinedAmnt[i + 1] + (perm[i +
                    1] == 0 ? 1 : 0);
        }
        int totalUndef = undefinedAmnt[0] + (perm[0] == 0 ? 1 :
                0);
        long[] bin = new long[n];
        bin[n - 1] = 1;
        ...
        for (int i = n - 2; i >= 0; i--) {
            if (undefinedAmnt[i] == undefinedAmnt[i + 1]) {
                ...
                bin[i] = (((bin[i + 1] * chisl) % MOD) *
                        inverseElmnt(znam))% MOD;
                chisl--;
                znam++;
            }
            if (perm[i] != 0) {
                incr[i] = perm[i + 1] != 0 ? incr[i + 1] :
                        currentIncr;
            } else {
                if (0 == currentIncr) {
                    currentIncr = 1;
                } else {
                    currentIncr = (((currentIncr * chislForIncr)
                            % MOD) * inverseElmnt(znamForIncr)) % MOD;
                    chislForIncr--;
                    znamForIncr++;
                }
            }
        ...
        for (int i = cell - 1; i >= 0; i--) {
            if (perm[i] == 0) {
                colSum[i] = (((colSum[i + 1] * chislColSum) %
                        MOD) * inverseElmnt(znamColSum)) % MOD;
                chislColSum--;
                znamColSum++;
                rowSum[i] = (((rowSum[i + 1] * chislRowSum) %
                        MOD) * inverseElmnt(znamRowSum)) % MOD;
                chislRowSum--;
                znamRowSum++;
            } else {
                colSum[i] = colSum[i + 1];
        ...
        for (int i = cell - 1; i >= 0; i--) {
            if (perm[i] != 0) {
                Treap res = new Treap(perm[i], generateY(),
                        null, null);
                Treap[] splitRes = t.split(perm[i]);
                lessAmntLeft[perm[i]] = splitRes[0] == null ? 0
                        : splitRes[0].size;
                if (null != splitRes[0]) {                     res = merge(splitRes[0], res);
                }
                if (null != splitRes[1]) {
        ...
        for (int i = 0; i < defValsSize; i++) {
            ...
            int greaterCnt = n - definedValue - (defValsSize - i
                    - 1);
        ...
        for (int i = n - 1; i >= 0; i--) {
            if (perm[i] != 0) {
                resultInpt[i] = (((incr[i] * (perm[i] - 1 -
                        smallerDefined[perm[i]])) % MOD) +
                        (lessAmntLeft[perm[i]] * bin[i]) % MOD)
                        % MOD;
            }
        }
        int undef = totalUndef;
        for (int i = 0; i < n; i++) {
            if (perm[i] == 0) {                 resultInpt[i] = (((((rowSum[i] * undef) % MOD) *
                        (undef - 1)) % MOD) * INV_TWO) % MOD;
                resultInpt[i] = (resultInpt[i] + (colSum[i] *
                        totalSum) % MOD) % MOD;
                undef--;
            } else {
                totalSum = (totalSum - greaterUndef[perm[i]] +
                        MOD) % MOD;
            }
        }
        ...
        for (int i = 1; i < n; i++) {
            ...
            resultInpt[i] = (resultInpt[i] * ((rightFact *
                    leftFact) % MOD)) % MOD;
        }
        long fact = 1;
        int cnt = 1;
 ...
 */    public static Treap merge(Treap l, Treap r) {
    ...
    private static class Treap {
        ...
        private Treap right;
        private int size;
        public Treap(final int x, final long y, final Treap
                left, final Treap right) {
            this.x = x;
            this.y = y;
            this.right = right;
            this.left = left;
 ...
 */        public Treap[] split(int x) {
            ...
            if (x < this.x) {
                if (this.left == null) {
                    newRight = new Treap(this.x, this.y,
                            this.left, this.right);
                } else {
                    Treap[] splitResult = this.left.split(x);
                    newLeft = splitResult[0];
                    newRight = new Treap(this.x, this.y,
                            splitResult[1], this.right);
                }
            } else {
                if (this.right == null) {
                    newLeft = new Treap(this.x, this.y,
                            this.left, this.right);
                } else {
                    Treap[] splitResult = this.right.split(x);
                    newLeft = new Treap(this.x, this.y,
                            this.left, splitResult[0]);
