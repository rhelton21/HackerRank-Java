...
class Regex {
    ...
    Regex(String regex) {
        ...
        parser.parse();
        SubsetConstruction subset = new
                SubsetConstruction(parser.expr.start, parser.expr.end);
    ...
    class SubsetConstruction {
        ...
        private Set<Node> eClosure(Node node) {
            return
                    eClosure(Collections.singletonList(node));
        }
        ...
        private Collection<Node> next(Collection<Node>
                                              nodes, char c) {
    ...
    class Parser {
        ...
        private Expression pipe(Expression first) {
            ...
            first.end.edges.add(new Edge(e, expr.end = new
                    Node()));
        ...
        private Expression star(Expression inner) {
            ...
            inner.end.edges.add(new Edge(e, expr.end = new
                    Node()));
        ...
        private boolean skipWs() {
            while (pos < regex.length() &&
                    Character.isWhitespace(regex.charAt(pos))) ++pos;
...
class RegexCounter {
    ...
    RegexCounter(Regex regex) {
        ...
        this.adjacencyMatrix = new long[regex.nodes.length]
                [regex.nodes.length];
    ...
    private int getIndex(Map<Regex.Node, Integer> indexes,
                         Regex.Node node) {
    ...
    long count(int len) {
        long[][] m = new
                MatrixPower(adjacencyMatrix).power(len);
        long count = 0;
        for (int finalState : finalStates)
            count = (count + m[0][finalState]) %
                    1000000007L;
        return count;
    }
}
class MatrixPower {
    ...
    long[][] power(int p) {
        ...
        if (result != null)
            return result;
        result = power(p / 2);
        powers.put(p / 2 * 2, result = multiply(result,
                result));
        if (p % 2 > 0)
            powers.put(p, result = multiply(result, power(p
                    % 2)));
        return result;
    }
...
public class CountStrings  {
    ...
    static long countStrings(String r, int l) {
        return l == 0 ? 0 : new RegexCounter(new
                Regex(r)).count(l);
    }
